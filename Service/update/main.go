...
func SetNewVerCallback(f func(string)) {
	callback = f
}
...
func _lanZouParseConnect(id string, u *url.URL) int {
	a, _ := http.NewRequest("GET", "https://"+u.Host+"/"+id, nil)

	a.Header.Set("Host", u.Host)
	a.Header.Set("Sec-Fetch-Dest", "document")
	a.Header.Set("Cache-Control", "max-age=0")
	a.Header.Set("Upgrade-Insecure-Requests", "1")
	a.Header.Set("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9")
	a.Header.Set("Sec-Fetch-Mode", "navigate")
	a.Header.Set("Connection", "keep-alive")
	a.Header.Set("sec-ch-ua-mobile", "?0")
	a.Header.Set("Sec-Fetch-Site", "none")
	a.Header.Set("Sec-Fetch-User", "?1")
	a.Header.Set("Referer", u.String())
	a.Header.Set("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Safari/537.36")
	a.Header.Set("Accept-Language", "zh-CN,zh;q=0.9")
	Response, _ := http.DefaultClient.Do(a)
	if Response == nil {
		return -1
	}
...
func _lanZouParseId(data string, pass string, u *url.URL) (bool, string, string) {
	a, _ := http.NewRequest("POST", "https://"+u.Host+"/filemoreajax.php", strings.NewReader(data+"&pwd="+pass))
	a.Header.Set("Host", u.Host)
	a.Header.Set("Sec-Fetch-Dest", "document")
	a.Header.Set("Cache-Control", "max-age=0")
	a.Header.Set("Upgrade-Insecure-Requests", "1")
	a.Header.Set("Accept", "application/json, text/javascript, */*")
...
func _lanZouParse(u *url.URL) string {
	a, _ := http.NewRequest("GET", u.String(), nil)
	a.Header.Set("Host", u.Host)
	a.Header.Set("Sec-Fetch-Dest", "document")
	a.Header.Set("Cache-Control", "max-age=0")
	a.Header.Set("Upgrade-Insecure-Requests", "1")
