...
export default {
  ...
  mounted() {
    ...
    this.Function.Save = () => {
      ...
      const x = this.GetCode()
      if (x.length < 5) {
        AppSaveGenerateCodeInterface("").then(() => {
          AppGetGenerateCodeInterface().then(res => {
            this.SetCode(res)
          })
        })
      ...
      AppSaveGenerateCodeInterface(x).then(() => {
