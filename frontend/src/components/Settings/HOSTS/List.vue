...
export default {
  ...
  data() {
    return {
      ...
      //当前选中行
      agSelectedLine: null,
      overlayNoRowsTemplate: `<span style="padding: 20px;" id="HookMessageText">无HOST映射</span>`,
      ...
      MenuItems: [
        {
          name: '增加',
          action: () => {
            this.AddLine("baidu.com", "www.baidu.com", "未保存")
            this.IsHasModify = true
          },
          disabled: false,
          visible: true
        ...
        },
        {
          name: '保存变更',
          action: () => {
            CallGoDo("保存HOSTS规则", {Data: this.RowData}).then(res => {
  ...
  ,
  methods: {
    AddLine(name, value, value2) {
      let obj = {}
      obj[this.columns[0].field] = name
      obj[this.columns[1].field] = value
      obj[this.columns[2].field] = value2
    ...
    AddLines(objs) {
      ...
      for (let i = 0; i < objs.length; i++) {
        ...
        let obj = {}
        obj[this.columns[0].field] = Src
        obj[this.columns[1].field] = Dest
        obj[this.columns[2].field] = "已保存"
        obj.Hash = (new Date().getTime()) + "";
        array.push(obj)
      }
      this.RowData = array
